...
# حفظ التقارير
with open("validation_report_rf.txt", "w") as f:
    f.write(classification_report(y_val, y_val_pred, digits=4))
with open("test_report_rf.txt", "w") as f:
    f.write(classification_report(y_test, y_test_pred, digits=4))

# حفظ الموديل
joblib.dump(final_model, "rf_best_model_final.pkl")
...
with open("top_features_rf.txt", "w", encoding="utf-8") as f:
...
plt.legend()
plt.grid(True)
plt.savefig("roc_curve_test_rf.png")
print("📈 ROC curve saved as roc_curve_test_rf.png")

print("\n✅ All done with Random Forest!")
