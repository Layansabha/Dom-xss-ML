...
# إعدادات
threads = 16  # ما بتستخدم مباشرة في AdaBoost، بس موجود للتوحيد
random_state = 42
k_folds = 5
...
final_model = AdaBoostClassifier(
    base_estimator=DecisionTreeClassifier(max_depth=grid.best_params_["base_estimator__max_depth"], random_state=random_state),
...
# حفظ التقارير
with open("validation_report_adaboost.txt", "w") as f:
    f.write(classification_report(y_val, y_val_pred, digits=4))
with open("test_report_adaboost.txt", "w") as f:
    f.write(classification_report(y_test, y_test_pred, digits=4))

# حفظ الموديل
joblib.dump(final_model, "adaboost_best_model_final.pkl")

# حفظ أهم الميزات (ممكن تكون أقل وضوحًا من XG/LGBM)
vocab = joblib.load(vocab_path)
importances = final_model.feature_importances_
top_tokens = sorted([(token, importances[idx]) for token, idx in vocab.items()],
                    key=lambda x: x[1], reverse=True)[:50]

with open("top_features_adaboost.txt", "w", encoding="utf-8") as f:
...
plt.legend()
plt.grid(True)
plt.savefig("roc_curve_test_adaboost.png")
print("📈 ROC curve saved as roc_curve_test_adaboost.png")

print("\n✅ All done with AdaBoost!")
